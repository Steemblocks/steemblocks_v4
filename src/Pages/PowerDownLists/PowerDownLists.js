...
const PowerDownLists = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          'https://steemdata.justyy.workers.dev/?data=ranking&by=powerdown',
          { timeout: 10000 }
        );
  ...
  useEffect(() => {
    if (powerDownData.length > 0 && Object.keys(profileImages).length === 0) {
      const initialItems = powerDownData.slice(0, itemsPerPage);
      fetchProfileImages(initialItems);
    }
  }, [powerDownData, profileImages, fetchProfileImages, itemsPerPage]);

  // Fetch profile images when page changes
  useEffect(() => {
    if (powerDownData.length > 0) {
  ...
  }, [currentPage, powerDownData, profileImages, fetchProfileImages, itemsPerPage]);

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = powerDownData.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(powerDownData.length / itemsPerPage);
  ...
  if (loading) {
    return (
      <div className="dashboard-container">
        <Loading
          message="Loading Power Down Data..."
          size="medium"
          variant="primary"
        />
      </div>
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="header-content">
        <h1 className="modern-title">Power Down Rankings</h1>
        <p className="modern-subtitle">
          Steem accounts with active power down transactions
        </p>
      </div>

      {/* Main Power Down Table */}
          ...
          <table className="headers-reference-table">
            ...
            <tbody>
              {currentItems.map((data, index) => (
                <tr key={index} className="reference-row">
                  ...
                  <td className="ref-powerdown">{formatNumber(data.powerdown || 0)} SP</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="pagination-container">
        <div className="pagination-info">
          <span className="pagination-text">
            Showing <strong>{indexOfFirstItem + 1}-{Math.min(indexOfLastItem, powerDownData.length)}</strong> of <strong>{powerDownData.length}</strong> accounts
          </span>
        </div>
