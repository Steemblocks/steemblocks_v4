...
const AccountRanking = () => {
  ...
  }, []); // Remove profileImages dependency to prevent infinite loop
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setRankingData([]); // Set empty array on error to prevent infinite loading
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    if (rankingData.length > 0 && Object.keys(profileImages).length === 0) {
      const initialItems = rankingData.slice(0, itemsPerPage);
      fetchProfileImages(initialItems);
    }
  }, [rankingData, profileImages, fetchProfileImages, itemsPerPage]);
  ...
  useEffect(() => {
    if (rankingData.length > 0) {
      ...
      // Only fetch images for accounts we don't already have
  ...
  }, [currentPage, rankingData, profileImages, fetchProfileImages, itemsPerPage]);
  ...
  if (loading) {
    return (
      <div className="dashboard-container">
        <Loading
          message="Loading Account Rankings..."
          size="medium"
          variant="primary"
        />
      </div>
  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="header-content">
        <h1 className="modern-title">Account Rankings</h1>
        <p className="modern-subtitle">
          Top Steem accounts ranked by total value in USD
        </p>
      </div>
          ...
          <tbody>
            {currentItems.map((data, index) => (
              <tr key={index} className="reference-row">
                ...
                <td className="ref-balance">{formatCurrency(data.usd || 0)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      <div className="pagination-container">
        <div className="pagination-info">
          <span className="pagination-text">
            Showing <strong>{indexOfFirstItem + 1}-{Math.min(indexOfLastItem, rankingData.length)}</strong> of <strong>{rankingData.length}</strong> accounts
          </span>
        </div>
